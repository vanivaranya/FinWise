...
import { Plus, Filter, Target, Calculator, TrendingUp } from "lucide-react";
...
const SavingsPage = () => {
  ...
  const addEntry = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(errorData.message || "Failed to add entry");
        return;
      }

      const data = await res.json();
      setEntries([...entries, data]);
      toast.success("New savings entry added");
    } catch {
      toast.error("Server error while adding entry");
    }
  };
  ...
  // ✅ Debounced budget toast
  const showBudgetToast = debounce((value) => {
    toast.info(`Savings target set to ₹${value}`);
  ...
  return (
    <div>
      {percentage >= 80 && (
        <div className="mt-3 p-2 bg-green-100 text-green-700 rounded-lg text-sm">
          Great! 🎉 You’ve achieved {percentage}% of your savings target.
        </div>
      )}
